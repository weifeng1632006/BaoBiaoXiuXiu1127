...
const app = createApp({
    ...
    setup() {
        ...
        const treeRef = ref([treeRef1, treeRef2]);
        ...
        const handleSearch = () => {
            console.log('这里开始搜查----------------');
            console.log('treeRef1', treeRef1.value);
            console.log('treeRef2', treeRef2.value);
            matchedCount.value = 0;
            if (treeRef1.value) {
                treeRef1.value.filter(searchText.value);
                if (autoExpand.value && searchText.value) {
                    expandMatchedNodes(data['chengben'].value);
                }
            }
            if (treeRef2.value) {
                treeRef2.value.filter(searchText.value);
                if (autoExpand.value && searchText.value) {
                    expandMatchedNodes(data['shechangbb'].value);
                }
            }
        };
        ...
        const handleAutoExpand = () => {
            ...
            treeRef.value.forEach(tree => {
                if (tree.value) {
                    const allNodes = tree.value.store._getAllNodes();
                    if (value) {
                        // 展开所有节点
